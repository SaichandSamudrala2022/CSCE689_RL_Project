...
import MST as tree
#import keras, tensorflow as tf,  # Vasudev 12/2
import numpy as np, sys, copy, argparse, random
import operator
import math
import os, time
#import DQN_Implementation  # Vasudev 12/2
...
#tf.random.set_seed(10701) # Vasudev 12/2
random.seed(10701)
...
def DRL_implementation(filename,globali):
    ...
    #        print('Routing net No.',sortedHalfWireLength[i][0])
        ...
        #gpu_ops = tf.GPUOptions(allow_growth=True)  # V:commented
        gpu_ops = tf.compat.v1.GPUOptions(allow_growth=True) # V: added
        #config = tf.ConfigProto(gpu_options=gpu_ops)  # V:commented
        config = tf.compat.v1.ConfigProto(gpu_options=gpu_ops)  # V: added
        #sess = tf.Session(config=config)  # V: commented
        sess = tf.compat.v1.Session(config=config)  # V: added

        # Setting this as the default tensorflow session.
        #keras.backend.tensorflow_backend.set_session(sess)  # V: commented
        tf.compat.v1.keras.backend.set_session(sess)  # V: added
        '''
    ...
    # graphcase.net_pair = twoPinNumEachNet
        ...
        #env = graph.GridGraph(init.gridParameters(grid_info))
        if RL_algo == 'PPO':
            #n_steps=2048
            #n_epochs=10
            n_timesteps=1000000
            #tmp_path = './logs/PPO_tensorboards_'+str(benchmark_number)+'/'
            #new_logger = configure(tmp_path, ["csv", "tensorboard"])
            model = PPO("MlpPolicy", graphcase, learning_rate=0.003,verbose=1,device="auto")
            model.learn(total_timesteps=n_timesteps)
        elif RL_algo == 'A2C':
            n_timesteps=1000000
            #tmp_path = './logs/A2C_tensorboards_'+str(benchmark_number)+'/'
            #new_logger = configure(tmp_path, ["csv", "tensorboard"])
            model = A2C("MlpPolicy", graphcase,verbose=1,device="auto")
            model.learn(total_timesteps=n_timesteps)
        elif RL_algo == 'TRPO':
            n_timesteps=1000000
            #tmp_path = './logs/TRPO_tensorboards_'+str(benchmark_number)+'/'
            #new_logger = configure(tmp_path, ["csv", "tensorboard"])
            model = TRPO("MlpPolicy", graphcase,verbose=1,device="auto")
            model.learn(total_timesteps=n_timesteps)
            
        
        posTwoPinNum = graphcase.posTwoPinNum
        
        if posTwoPinNum >= len(graphcase.twopin_combo): #twoPinNum: 
            print("RL agent found better solution")
        ...
        agent.sess.close()
        #tf.reset_default_graph() # Vasudev 12/2
